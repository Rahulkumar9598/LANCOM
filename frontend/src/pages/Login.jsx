...
const LoginConfig = {
  ...
  hero: {
    ...
    features: [
      { icon: "UserCheck", title: "Role-Based Access", description: "Admin | Supervisor | Operator", color: "blue" },
      { icon: "Mail", title: "Real-time Updates", description: "Live instructions & notifications", color: "blue" },
      { icon: "Phone", title: "Multi-Channel Alerts", description: "Email | WhatsApp | Push", color: "blue" },
    ],
  },
  form: {
    email: { label: "Email Address", placeholder: "admin@factory.com", type: "email", required: true },
    password: { label: "Password", placeholder: "••••••••", required: true },
    rememberMe: { label: "Remember me", defaultChecked: false },
    forgotPassword: { label: "Forgot Password?", link: "#" },
  ...
  demoAccounts: {
    ...
    accounts: [
      { id: "admin", email: "admin@factory.com", label: "Admin", color: "purple", role: "Admin", department: "Admin" },
      { id: "supervisor", email: "supervisor@factory.com", label: "Supervisor", color: "blue", role: "Supervisor", department: "QC" },
      { id: "operator", email: "operator@factory.com", label: "Operator", color: "green", role: "Operator", department: "QC" },
    ],
  },
  background: {
    type: "gradient",
...
// Helper to retrieve icon component
...
const Login = () => {
  ...
  const validateForm = () => {
    ...
    else if (!LoginConfig.validation.emailPattern.test(email)) errors.email = "Please enter a valid email address";
    if (!password) errors.password = "Password is required";
    else if (password.length < LoginConfig.validation.minPasswordLength) errors.password = `Password must be at least ${LoginConfig.validation.minPasswordLength} characters`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await API.post(ConstantApi.auth.login, { email: email.trim(), password: password.trim() });
      if (res.data.success) {
        ...
        setTimeout(() => {
          navigate(res.data.department?.role === "superadmin" ? "/superadmin/dashboard" : "/dashboard");
        }, 500);
      } else {
        showToast(res.data.error || LoginConfig.messages.loginFailed, "error");
      }
    } catch (err) {
      showToast(err.response?.data?.message || LoginConfig.messages.loginFailed, "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const quickLogin = (userEmail) => {
    setEmail(userEmail);
    setPassword(LoginConfig.demoAccounts.defaultPassword);
    setFormErrors({});
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-yellow-400 rounded-full blur-3xl" />
      </div>
      <div className="relative flex items-center justify-center min-h-screen p-4">
        <div className="max-w-6xl w-full grid md:grid-cols-2 gap-8 items-center">
          ...
          <div className="hidden md:block text-white space-y-6">
            ...
            <p className="text-blue-100 text-lg">{LoginConfig.hero.description}</p>
            <div className="space-y-4 pt-8">
              {LoginConfig.hero.features.map((f, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className={`w-10 h-10 bg-${f.color}-500/30 rounded-lg flex items-center justify-center`}>
                    <DynamicIcon name={f.icon} className={`text-${f.color}-300 w-5 h-5`} />
                  </div>
                  <div>
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8 md:p-10">
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{LoginConfig.form.email.label}</label>
                <div className="relative">
                  <Icons.Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                  <input type={LoginConfig.form.email.type} value={email} onChange={e => setEmail(e.target.value)}
                    className={`w-full pl-10 pr-4 py-3 border ${formErrors.email ? "border-red-500" : "border-gray-300"} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition`} placeholder={LoginConfig.form.email.placeholder} required={LoginConfig.form.email.required} />
                  {formErrors.email && <p className="mt-1 text-xs text-red-500">{formErrors.email}</p>}
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{LoginConfig.form.password.label}</label>
                <div className="relative">
                  <Icons.Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                  <input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)}
                    className={`w-full pl-10 pr-12 py-3 border ${formErrors.password ? "border-red-500" : "border-gray-300"} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition`} placeholder={LoginConfig.form.password.placeholder} required={LoginConfig.form.password.required} />
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500" />
                  <span className="text-sm text-gray-600">{LoginConfig.form.rememberMe.label}</span>
                </label>
                <a href={LoginConfig.form.forgotPassword.link} className="text-sm text-blue-600 hover:text-blue-700 font-medium">{LoginConfig.form.forgotPassword.label}</a>
              </div>
              <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-xl font-semibold hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-lg shadow-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? (
            ...
            {LoginConfig.demoAccounts.enabled && (
              <div className="mt-8 pt-6 border-t border-gray-200">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {LoginConfig.demoAccounts.accounts.map(acc => (
                    <button key={acc.id} onClick={() => quickLogin(acc.email)} className={`px-2 py-1.5 text-xs bg-${acc.color}-50 text-${acc.color}-700 rounded-lg hover:bg-${acc.color}-100 transition`}>{acc.label}</button>
                  ))}
                </div>
